...
RUN yum install -y java-1.8.0-openjdk which unzip openssh-server sudo openssh-clients hostname && yum clean all
...
WORKDIR /home/activemq/apache-activemq-5.9.0/conf
RUN rm -f startup.sh
RUN curl   --output startup.sh  https://raw.githubusercontent.com/amitoj/amq-docker/master/activemq-cluster-config.sh 
